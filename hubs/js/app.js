...
async function init(){
  const res=await fetch('./data/hubs.json?v=20260603-buttonfix',{cache:'no-store'});
  DATA=await res.json();
...
function render(slug){
  ...
  $('map').src=(h.map||'')+'?v=20260603-buttonfix';
  ...
  history.replaceState(null,'',`?hub=${h.slug||h.id}&v=20260603-buttonfix`);
}
function renderPending(h){
  $('title').textContent=`${h.icon||''} ${h.title}`;
  $('subtitle').textContent=h.subtitle||'제작 예정';
  $('verse').textContent='이 허브는 다음 단계에서 제작할 예정입니다.';
  $('map').src='assets/maps/abraham-hub-map.png?v=20260603-buttonfix';
